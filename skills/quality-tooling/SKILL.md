...
| **a11y test** | `npm run test:a11y <url>` | WCAG 2.0–2.2 A+AA via axe-core against the rendered page; **fails on missing alt text**, contrast, ARIA, heading order | The Alt-Text Rule · `accessibility` |
...
2. **Any UI change** → start the dev server (`npx aem up`), then `npm run test:a11y http://localhost:3000/<path>` (or a branch-preview URL).
3. **Any code change** → `npm run lint`.
4. A non-zero exit = a real violation. Read the output, fix per the named skill, re-run until green. Never loosen a checker to make it pass.
