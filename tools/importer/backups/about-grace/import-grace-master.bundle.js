...
var CustomImportScript = (() => {
  ...
  var catalog_data_default = {
    ...
    "blocks": [
      ...
      },
      {
        ...
        "family": "columns",
        "render": "parse",
        "selector": 'section.none-bkgd .row:has(> [class*="col-lg-6"] .button):has(> [class*="col-lg-6"] .image)',
        "item": null,
        "note": "address+jobs CTA+image; tightened to the inner two-half row (not the col-lg-3/9 layout wrapper) so the location PHOTO cell is captured",
        "priority": 20
      },
      {
        "name": "columns-people",
  ...
  function parse(element, { document }) {
    ...
    let heading = element.querySelector(".hero__heading h1, .hero__heading h2, .hero__heading h3") || element.querySelector("h1") || element.querySelector("h2");
  ...
  function parse26(element, { document }) {
    ...
    if (cols.length >= 2) {
      ...
      const isImageCell = (nodes) => nodes.length && nodes.every((n) => n.tagName === "PICTURE" || n.tagName === "IMG" || n.querySelector && n.querySelector("picture, img") && !(n.textContent || "").replace(/\s+/g, " ").trim());
  ...
  var MATCHERS = {
    ...
    "table-data-grid": (doc) => {
      const cookieGrids = Array.from(doc.querySelectorAll(".rich-text:not(.vertical-border) > table[width='100%']")).filter((t) => {
        const r = t.querySelector("tr");
        return r && r.children.length === 5;
      });
      const featureTables = Array.from(doc.querySelectorAll(".rich-text.vertical-border > table")).filter((t) => {
        const r = t.querySelector("tr");
  ...
  function isAboutGraceTextSidebar(document, url) {
    ...
    const hasBlocks = !!art.querySelector(
      '.cmp-card-list, .cmp-card, .media-callout, .cmp-media-callout, .feature-set, .featured-blog-cmp, .accordion-comp, table, .cmp-image, [data-cmp-is="image"]'
    );
    const hasContentImg = Array.from(art.querySelectorAll("img, picture")).some((im) => !im.closest("header, footer, nav, .section-navigation"));
  ...
  function buildSidebarNav(document) {
    const canon = (document.querySelector('link[rel="canonical"]') || {}).href || (document.querySelector('meta[property="og:url"]') || {}).content || "";
    const crumbHrefs = Array.from(document.querySelectorAll('.cmp-breadcrumb a, nav[aria-label*="readcrumb" i] a')).map((a) => a.getAttribute("href") || "").join(" ");
  ...
  function materializeLazyImages(document) {
    document.querySelectorAll('[data-cmp-is="image"], [data-cmp-src], [data-eds-src]').forEach((el) => {
      const root = el.hasAttribute("data-cmp-src") || el.hasAttribute("data-eds-src") ? el : el.querySelector("[data-cmp-src], [data-eds-src]") || el;
      ...
      if (root.hasAttribute("data-eds-src") && !root.querySelector("img")) {
        ...
        img2.setAttribute("src", edsSrc);
        const edsAlt = (root.getAttribute("data-eds-alt") || "").trim();
        if (edsAlt) {
          img2.setAttribute("alt", edsAlt);
        } else {
          let base = "";
          try {
            base = new URL(edsSrc).pathname.split("/").pop().replace(/\.[a-z0-9]+$/i, "");
          } catch (e) {
            base = "";
          }
          img2.setAttribute("alt", base.replace(/[-_]+/g, " ").trim().replace(/\b\w/g, (c) => c.toUpperCase()));
        }
        (root.querySelector(".cmp-image") || root).appendChild(img2);
        return;
      }
  ...
  function sectionizeFlatBody(main, document, splitFingerprints = []) {
    ...
    const sections = [];
    let pending = [];
    const flushBody = (body) => {
      splitRun(body).forEach((r) => {
        if (r.length) sections.push(r);
      });
    };
    seq.forEach((item) => {
      if (item.type === "leaf") {
  ...
  function buildDefaultPage(document, url, params) {
    ...
    if (isIndustries) {
      ...
      if (!crumb) {
        try {
          const segs = new URL(params && params.originalURL || url).pathname.replace(/\/$/, "").split("/").filter(Boolean);
    ...
    if (params && params.industriesNav && params.industriesNav.children.length) {
      const navSection = params.industriesNav;
      ...
      if (heroSection) {
        ...
        main.insertBefore(document.createElement("hr"), navSection);
      } else {
    ...
    if (params && params.industriesNav) {
      Array.from(main.children).forEach((c) => {
        if (c.nodeType === 1 && c.tagName === "DIV" && !(c.textContent || "").trim() && !c.querySelector("img, picture, table, ul, ol, a, hr")) {
          c.remove();
        }
      });
      Array.from(main.children).forEach((c) => {
  ...
  var import_grace_master_default = {
    ...
    onLoad: (_0) => __async(void 0, [_0], function* ({ document }) {
      ...
      const imgDeadline = Date.now() + 6e3;
      const imgReady = () => {
        const comps = [...document.querySelectorAll('article .cmp-image, article [data-cmp-is="image"]')].filter((c) => !c.closest("header, footer, nav"));
      ...
      while (!imgReady() && Date.now() < imgDeadline) {
        yield new Promise((r) => {
          setTimeout(r, 300);
        });
      }
      document.querySelectorAll('[data-cmp-src], [data-cmp-is="image"], .cmp-image').forEach((el) => {
        ...
        const alt = (root.getAttribute("data-title") || root.getAttribute("alt") || root.querySelector("img") && root.querySelector("img").getAttribute("alt") || "").replace(/<[^>]*>/g, "").trim();
    ...
    transform: (payload) => {
      ...
      params.sourceFeaturedHasGeoHex = !!(featuredBlogEl && (featuredBlogEl.closest(".geoAndHex") || pageHasGeoAndHex));
      params.sourceFeaturedIsPlainGray = !!(featuredBlogEl && !params.sourceFeaturedHasGeoHex && featuredBlogEl.closest(".light-gray-bkgd"));
