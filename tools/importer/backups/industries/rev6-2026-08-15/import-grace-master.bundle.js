...
var CustomImportScript = (() => {
  ...
  function sectionizeFlatBody(main, document, splitFingerprints = []) {
    ...
    const splitRun = (run) => {
      ...
      run.forEach((el, idx) => {
        ...
        if ((fpBoundary || imgBoundary) && cur.length) {
          out.push(cur);
          cur = [];
        }
        cur.push(el);
  ...
  var import_grace_master_default = {
    ...
    transform: (payload) => {
      ...
      params.sourceFeaturedIsPlainGray = !!(featuredBlogEl && !featuredBlogEl.closest(".geoAndHex") && featuredBlogEl.closest(".light-gray-bkgd"));
